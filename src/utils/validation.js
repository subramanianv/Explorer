...
// Decorators
const stringInputDecorator = wrappedFun => wrappedFunArgs => (
  value,
  allValues
) => {
...
const dateRequired = value =>
  value[0] !== '' && value[1] !== '' ? undefined : 'Required Field';
...
const isNumber = value =>
  value && isNaN(Number(value)) ? strings.IS_NUMBER : undefined;
const isCurrency = value =>
  value && !/^\d+(?:\.\d{0,2})*$/.test(value) ? strings.IS_CURRENCY : undefined;
const isEmail = value =>
...
const lessThan = (ltkey, ltDisplay) => (value, allValues) => {
  ...
  return value <= (parseInt(allValues[ltkey], 10) || Infinity)
    ? undefined
    : msg;
};
...
const notIn = collection => value =>
  includes(collection, value) ? strings.NAME_EXISTS : undefined;
...
const fileSizeCheck = (value, allValues) => {
  ...
  if (actualSize !== value) {
    return stringFormat(strings.FILE_SIZE, {
      value,
      actual_size: allValues.actual_size
    });
  }
  return undefined;
};

const fileListCheck = fileList =>
...
export const errors = {
  ...
  validNameList: [
    stringInputDecorator(maxLength)(30),
